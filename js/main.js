$(function(){
    ...
    var digitalnz_api_url = "http://api.digitalnz.org/v3/records.json?api_key=" + digitalnz_api_key;
    ...
    function get_article() {
        if ( current_cell <= cells ) {
            ...
            if (keywords === "" && !$.isEmptyObject(totals)) {
                ...
                query = digitalnz_api_url + "&text=" + keywords + "&and[year]=" + current_year;
                if (item_type == 'images') {
                    query += "&and[category]=Images";
                } else if (item_type == 'newspapers') {
                    query += "&and[display_collection][]=Papers+Past";
                }
    ...
    function make_query(number) {
        ...
        url = digitalnz_api_url + "&text=" + keywords + "&and[year]=" + current_year + "&per_page=1&page=" + number;
        if (item_type == 'images') {
            url += "&and[category]=Images";
        } else if (item_type == 'newspapers') {
            url += "&and[display_collection][]=Papers+Past";
        }
        return url;
    }
    ...
    function process_results(results, type) {
        var article;
        if (type == 'total') {
            //var total = results.search.zone[0].records.total;
            var total = results.search.result_count;
            if (total > 0) {
                var number = Math.floor(Math.random() * total) + 1;
                var query = make_query(number);
                get_api_result(query, 'article');
            } else {
                display_dnz_article(null);
            }
        } else if (type == 'article') {
            //if (results.response.zone[0].records.total > 0) {
            //    article = results.response.zone[0].records.article[0];
            if (results.search.result_count > 0) {
                article = results.search.results[0];
            } else {
                article = null;
            }
            display_dnz_article(article);
        }
    }

    function display_dnz_article(article) {
        var $cell = $('<div id="cell' + current_cell + '-content" ></div>');
        if (article) {
            var page_id, heading;
            var article_id = article.id;
            var date = $.datepicker.formatDate("d MM yy", new Date(Date.parse(article.date)));
            if (article.title.length > 50) {
                heading = article.title.substring(0, 50) + "...";
            } else {
            ...
            if (article.thumbnail_url) {
                $front_content.append('<div class="img_holder" ><img src="' + article.thumbnail_url + '"></div>');
            } else {
                $front_content.append('<div class="title_holder" ><div class="news_title">' + get_title(heading) + '</div></div>');
            }
            $cell.append($front_content);
            $back_content = $('<div class="back"></div>');
            $details = $('<div class="details"></div>');
            $details.append("<p><b>" + heading + "</b></p>");
            $details.append("<p>" + date + "</p>");
            $details.append('<p><a target="_blank" href="' + article.source_url + '">More...</a></p>');
            $back_content.append($details);
        ...
        $('*[data-value="' + current_cell + '"]').html($(".front", $cell).html()).hide().show("fade")
            .mouseenter(function() {
                ...
                $('.news_title', $(this)).fadeTo('fast', 1);
                $('.year', $(this)).fadeTo('fast', 0.2);
            })
            .mouseleave(function() {
                $('img', $(this)).fadeTo('fast', 0.5);
                $('.news_title', $(this)).fadeTo('fast', 0.5);
                $('.year', $(this)).fadeTo('fast', 1);
            });
        current_cell += 1;
        prepare_cell();
    }

    function display_trove_article(article) {
        var $cell = $('<div id="cell' + current_cell + '-content" ></div>');
        if (article) {
            var page_id, heading;
            var article_id = article.id;
    ...
    function get_title(heading) {
        ...
        if (heading.indexOf('(') > -1) {
            title = heading.substring(0, heading.indexOf('('));
        } else {
            title = heading;
        }
        return title;
    ...
    $(".cell").click(function() {
        ...
        $(this).flippy({
            ...
            onFinish: function() {
                ...
                $("#magic_total").html($("#magic_total").data('value'));
            }
        });
    });
